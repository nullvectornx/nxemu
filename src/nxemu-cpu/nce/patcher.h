...
public:
    ...
    bool PatchText(const uint8_t * program_image, uint64_t image_size, uint64_t code_offset, uint64_t code_size);
    bool RelocateAndCopy(uint64_t load_base, uint64_t code_offset, uint64_t code_size, uint8_t * program_image, uint64_t * image_size, EntryTrampolines * out_trampolines);
...
private:
    ...
    void WriteMrsHandler(ModuleDestLabel module_dest, oaknut::XReg dest_reg, oaknut::SystemReg src_reg);
